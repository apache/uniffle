...
class UniffleStageDependencyListener extends SparkListener with Logging {
  ...
  override def onStageSubmitted(stageSubmitted: SparkListenerStageSubmitted): Unit = {
    ...
    logInfo(s"stageId: $stageId, parentStageIds: $parentStageIds, shuffleId: $shuffleIdFromProducer")
    ...
    getShuffleManager().map(manager => {
      manager.getStageDependencyTracker.ifPresent(tracker => {
        ...
        shuffleIdFromProducer.map(shuffleId => tracker.linkWriter(shuffleId, stageId))
        ...
        parentStageIds.foreach(parentStageId => {
          ...
          tracker.linkReader(shuffleId, stageId)
        })
      })
    })
  }
